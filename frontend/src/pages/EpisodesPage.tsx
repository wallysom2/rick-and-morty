...
function EpisodeCard({ episode }: { episode: Episode }) {
  return (
    <Link 
      ...
      className="group relative flex flex-col bg-[var(--bg-card)] rounded-xl overflow-hidden border border-[var(--border-default)] hover:border-[var(--portal-cyan)]/50 hover:shadow-[0_0_20px_rgba(0,212,255,0.15)] transition-all duration-300 animate-fade-in-up"
    >
      <div className="p-5 flex flex-col h-full">
        <div className="flex items-start justify-between gap-3 mb-3">
          <span className="px-2.5 py-1 text-xs font-mono font-bold text-[var(--portal-cyan)] bg-[var(--space-medium)] rounded group-hover:bg-[var(--portal-cyan)]/10 transition-colors">
            {episode.episode}
          </span>
        ...
        <h3 className="text-lg font-semibold text-[var(--text-primary)] group-hover:text-[var(--portal-cyan)] transition-colors line-clamp-2 mb-auto">
          {episode.name}
        </h3>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 sm:py-24">
          <div className="relative mb-6">
            ...
            >
              <IoSearchOutline className="w-12 h-12 sm:w-16 sm:h-16 text-[var(--portal-cyan)]" />
            </div>
          </div>
