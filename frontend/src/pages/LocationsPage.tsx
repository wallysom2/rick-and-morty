    ...
    >
      <div className="p-5 flex flex-col h-full">
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="p-2 rounded-lg bg-[var(--space-medium)] text-[var(--dimension-purple)] transition-colors">
            {icon}
          </div>
        ...
        <h3 className="text-lg font-semibold text-[var(--text-primary)] group-hover:text-[var(--dimension-purple)] transition-colors line-clamp-2 mb-1">
          {location.name}
        </h3>
    ...
    <Container>
      ...
      <PageHeader
        ...
        subtitle={
          <p>
            Explore{' '}
            <span className="text-[var(--dimension-purple)] font-semibold font-mono">
              {pagination.count || 'todas as'}
            </span>{' '}
            localizações do multiverso Rick and Morty
          </p>
        }
        accentColor="var(--dimension-purple)"
      />
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 sm:py-24">
          <div className="relative mb-6">
            ...
            >
              <IoSearchOutline className="w-12 h-12 sm:w-16 sm:h-16 text-[var(--dimension-purple)]" />
            </div>
          </div>
          ...
          {pagination.pages > 1 && (
            <div className="mt-4 text-center">
              <span className="text-sm text-[var(--text-muted)]">
                Mostrando página{' '}
                <span className="text-[var(--dimension-purple)] font-semibold">{page}</span>
                {' '}de{' '}
                <span className="text-[var(--dimension-purple)] font-semibold">{pagination.pages}</span>
              </span>
            </div>
          )}
        </>
